...
namespace UniOrm.Application
{
    ...
    public class GodMaker : IGodMaker
    {
        ...
        public GodMaker(ICodeService codeService, IDbFactory dbFactory, IConfig config)
        {
            DbFactory = dbFactory;
            ...
            if (cons.RunMode == RunMode.Coding)
            {
                ...
                else
                {
                    ...
                    foreach (var s in steps)
                    {
                        ...
                        object model = Manager.RuntimeCache.GetOrCreate(cacheKey, entry =>
                      {
                          object newobj = null;
                          Manager.RuntimeCache.Set(cacheKey, newobj);
                          return newobj;
                      });
                        if (model == null || !s.IsUsingCache)
                        {
                                        ...
                                        case FlowStepType.Declare:
                                            {
                                                lock (lockobj)
                                                {
                                                    ...
                                                    var runcode = Manager.FindOrAddRumtimeCode(s.Guid);
                                                    ...
                                                    if (runcode == null)
                                                    {
                                                        ...
                                                        if (!string.IsNullOrEmpty(s.TypeLib))
                                                        {
                                                            ...
                                                            if (Manager.DynamicReferenceDlls.Contains(dllfile))
                                                            {
                                                                isref = false;
                                                            }
                                                            else
                                                            {
                                                                Manager.DynamicReferenceDlls.Add(dllfile);
                                                                isref = true;
                                                                dlls.Add(dllfile);
                                                            }
                                                        }
                                                        if (!string.IsNullOrEmpty(s.ReferenceDlls))
                                                        {
                                                            ...
                                                            {
                                                                Manager.DynamicReferenceDlls.Add(dllbase + n);
                                                            }
                                                        ...
                                                        Manager.RuntimeCodes.Add(s.Guid, runcode);
                                                    }
                                                    if (!string.IsNullOrEmpty(s.ReferenceDlls))
                                                    {
                                                        string dllbase = AppDomain.CurrentDomain.BaseDirectory;
                                        ...
                                        case FlowStepType.GetData :
                                            {
                                                ...
                                                object dbgrouder = Manager.RuntimeCache.GetOrCreate(cacheormnameKey, entry =>
                                                {
                                                    ...
                                                    Manager.RuntimeCache.Set(cacheKey, dbgroudernew);
                                                ...
                                                {
                                                    DynaObject = Manager.GetData(dbgrouder, s.InParamter1);
                                                }
                                                else
                                                {
                                                    DynaObject = Manager.GetData(dbgrouder, s.InParamter1, objParams2.ToArray());
                                                } 
                                            }
                                            break;
                                        case FlowStepType.CallMethod:
                                            {
                                                var methodsub = Manager.GetMethodFromConfig(s.IsBuildIn.Value, s.TypeLib, s.TypeFullName, s.MethodName);
                            ...
                            Manager.RuntimeCache.Set(cacheKey, rebject);
                        }
                        else
                        {
                            rebject = model;
        ...
        private ComposeEntity FindComposity(AppConfig appconfig, string allname)
        {
            lock (lockobj)
            {
                ComposeEntity cons = Manager.Composeentitys.FirstOrDefault(p => p.Guid == appconfig.StartUpCompoistyID);
                if (cons == null)
                {
                    cons = CodeService.GetConposity(appconfig.StartUpCompoistyID, allname).FirstOrDefault();
                    if (cons != null)
                    {
                        Manager.Composeentitys.Add(cons);
                    }
                }
        ...
        private ComposeTemplate FindComposeTemplet(string tid)
        {
            lock (lockobj)
            {
                var cons = Manager.ComposeTemplates.FirstOrDefault(p => p.Guid == tid);
                if (cons == null)
                {
                    cons = CodeService.GetSimpleCodeLinq<ComposeTemplate>(p => p.Guid == tid).FirstOrDefault();
                    if (cons != null)
                    {
                        Manager.ComposeTemplates.Add(cons);
                    }
                }
        ...
        private static IEnumerable<AConFlowStep> FindSteps(string ComId, ICodeService codeService)
        {
            ...
            var cons = Manager.AConFlowSteps.Where(p => p.AComposityId == ComId);
            if (cons == null || cons.Count() == 0)
            {
                cons = codeService.GetAConStateSteps(ComId).OrderBy(p => p.StepOrder).ToList();
                if (cons != null)
                {
                    Manager.AConFlowSteps.AddRange(cons);
                }
            }
