...
namespace AConStateFactory.Controllers
{
    ...
    public class FactController : ControllerBase
    {
        ...
        public IEnumerable GetTypes()
        {
            ...
            Assembly asm = null;
            if (!Manager.Dlls.ContainsKey(dllfile))
            {
                asm = Assembly.LoadFrom(dllfile);
                Manager.Dlls.Add(dllfile, asm);
            }
            else
            {
                asm = Manager.Dlls[dllfile];
            }
        ...
        public int UpdateCache()
        {
            Manager.Dlls.Clear();
            Manager.Types.Clear();
            Manager.MethodInfos.Clear();
            Manager.RuntimeCodes.Clear();
            Manager.Composeentitys.Clear();
            Manager.DynamicReferenceDlls.Clear();
            Manager.AConFlowSteps.Clear();
            Manager.ComposeTemplates.Clear();
            Manager.ClearCache();
            return 0;
        }
        [HttpPost]
